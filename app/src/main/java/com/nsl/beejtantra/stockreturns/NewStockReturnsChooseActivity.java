package com.nsl.beejtantra.stockreturns;
 ...
 */
public class NewStockReturnsChooseActivity extends AppCompatActivity {
    // JSON parser class
    ...
    String selected_company_id, selected_customer_id, sel_product_id, selected_division_id, finalJson, company_id, customer_id, order_date, status, orderidfromserviceorder;
    ...
    private Spinner spin_mo;
    private View view_mo;
    int role;
    private String sel_moId;
    String team;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar_title.setText("Stock Returns");
        ...
        tv_mo = (TextView) findViewById(R.id.text_mo);
        ...
        spin_mo = (Spinner) findViewById(R.id.spin_mo);

        view_mo = (View) findViewById(R.id.view_mo);
        ...
        if (role != Constants.Roles.ROLE_7) {
            view_mo.setVisibility(View.VISIBLE);
            spin_mo.setVisibility(View.VISIBLE);
            tv_mo.setVisibility(View.VISIBLE);
        ...
        btn_savebooing.setText("GO TO STOCK RETURNS");
        btn_savebooing.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (role != Constants.Roles.ROLE_7) {
                    if (selected_company_id == null || selected_company_id == "0" || selected_customer_id == null || selected_customer_id == "0" || selected_division_id == null || selected_division_id == "0" || sel_moId ==null || sel_moId == "0" || !auto_tv.getText().toString().trim().equalsIgnoreCase(customername)) {
                        Toast.makeText(NewStockReturnsChooseActivity.this, "Please select Company, Division and go to booking ", Toast.LENGTH_SHORT).show();
                    }
                /*else if (selected_customer_id == null || selected_customer_id == "0") {
                    Toast.makeText(NewAdvancebokingChooseActivity.this, "Please select Customer", Toast.LENGTH_SHORT).show();
                } else if (selected_division_id == null || selected_division_id == "0") {
                    Toast.makeText(NewAdvancebokingChooseActivity.this, "Please select Division", Toast.LENGTH_SHORT).show();
                }*/
                    else {
                        Intent cropsproducts = new Intent(getApplicationContext(), CropsFragmentStockReturnsActivity.class);
                        startActivity(cropsproducts);
                    }
                } else {
                    if (selected_company_id == null || selected_company_id == "0" || selected_customer_id == null || selected_customer_id == "0" || selected_division_id == null || selected_division_id == "0" || !auto_tv.getText().toString().trim().equalsIgnoreCase(customername)) {
                        Toast.makeText(NewStockReturnsChooseActivity.this, "Please select Company,Customer& Division and go to booking ", Toast.LENGTH_SHORT).show();
                    }
                /*else if (selected_customer_id == null || selected_customer_id == "0") {
                    Toast.makeText(NewAdvancebokingChooseActivity.this, "Please select Customer", Toast.LENGTH_SHORT).show();
                } else if (selected_division_id == null || selected_division_id == "0") {
    ...
    public class AsyncGetofflineCompanies extends AsyncTask<String, String, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
           /* progressDialog = new ProgressDialog(NewStockMovementChooseActivity.this);
          ...
          /*  if (progressDialog.isShowing())
            ...
            spin_company.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                ...
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    selected_company_id = organisations.get(position).getCityId();
                    Log.e("selected",selected_company_id);
                    ...
                    if (selected_company_id.equalsIgnoreCase("0")) {

                    } else {
                        companycode = organisations.get(position).getCityName();
                        companyname = organisations.get(position).getCityName();
                        String selectQuerys = "SELECT  " + KEY_TABLE_COMPANIES_COMPANY_CODE + " FROM " + TABLE_COMPANIES + " WHERE " + KEY_TABLE_COMPANIES_MASTER_ID + " = " + selected_company_id;
                        sdbw = db.getWritableDatabase();
                        ...
                        editor.putString("company_id", selected_company_id);
                        editor.commit();
        ...
        protected String doInBackground(String... params) {
           ...
           // adapter_crops.add(Common.getStringResourceText(R.string.select_distributor));
            try {
                ...
                String selectQuery = "SELECT  " + "CR." + KEY_TABLE_CUSTOMER_MASTER_ID + "," + KEY_TABLE_CUSTOMER_NAME + ",CDC." + KEY_TABLE_USER_COMPANY_CUSTOMER_CUSTOMER_ID +",CR."+KEY_TABLE_CUSTOMER_CITY+ " FROM " + TABLE_USER_COMPANY_CUSTOMER + " AS CDC JOIN " + TABLE_CUSTOMERS + " AS CR ON CDC." + KEY_TABLE_USER_COMPANY_CUSTOMER_CUSTOMER_ID + " = CR." + KEY_TABLE_CUSTOMER_MASTER_ID + "  where " + "CDC." + KEY_TABLE_USER_COMPANY_CUSTOMER_COMPANY_ID + " = " + selected_company_id + " and " + KEY_TABLE_USER_COMPANY_CUSTOMER_USER_ID + " = " + sel_moId + " and CR." + KEY_TABLE_USER_COMPANY_CUSTOMER_STATUS + " = 1"+ " and CDC." + KEY_TABLE_USER_COMPANY_CUSTOMER_STATUS + " = 1 group by(CR." + KEY_TABLE_CUSTOMER_MASTER_ID + ")";
                sdbw = db.getWritableDatabase();
                ...
                if (cursor.moveToFirst()) {
                    do {
                        ...
                        cities2.setCustomerCode(cursor.getString(2));
                        // System.out.println("city id is :" + cityId + "city name is :" + cityName);
          ...
          /*  if (progressDialog.isShowing())
           ...
           /* spin_customer.setAdapter(new ArrayAdapter<String>(getApplicationContext(), R.layout.spinner_item, R.id.customSpinnerItemTextView, adapter_crops));
            spin_customer.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                @Override
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    selected_customer_id = arlist_crops.get(position).getCustomerCode();
                    //  tv_customer.setTextColor(getResources().getColor(R.color.colorPrimary));

                    if (selected_customer_id.equalsIgnoreCase("0")) {

                    } else {
                        customername = arlist_crops.get(position).getCityName();
                        SharedPreferences.Editor editor = sharedpreferences.edit();
                        editor.putString("customer_id", selected_customer_id);
...
*/
            ...
            spin_division.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                ...
                public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    selected_division_id = arlist_schemes.get(position).getCityId();
                    if (selected_division_id.equalsIgnoreCase("0")) {

                    } else {
                        divisionname = arlist_schemes.get(position).getCityName();
                        SharedPreferences.Editor editor = sharedpreferences.edit();
                        editor.putString("division_id", selected_division_id);
                        editor.commit();
                        if (role != Constants.Roles.ROLE_7) {
                            callapi();
                        }else {
    ...
    private void callapi() {
        ...
        try {
            ...
            String selectQuery = "SELECT " + KEY_TABLE_USERS_FIRST_NAME + "," + KEY_TABLE_USERS_MASTER_ID + " FROM " + TABLE_USERS + "  WHERE " +db.KEY_TABLE_USERS_STATUS+" = 1 and user_id in (" + userId + "," + team + ")";
        ...
        spin_mo.setAdapter(new ArrayAdapter<String>(getApplicationContext(), R.layout.spinner_item, R.id.customSpinnerItemTextView, adapterusers));
        spin_mo.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {

            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                sel_moId = String.valueOf(userses.get(position).getID());
                Log.e("user spinner id",sel_moId);

                SharedPreferences.Editor editor = sharedpreferences.edit();
                editor.putString("mo_id", sel_moId);
                editor.commit();
    ...
    public void autoCompleteAdapter(List<SelectedCities> list) {
        ...
        adapter.setOnItemClickListener(new CustomListAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position, SelectedCities item) {
                ...
                Common.hideSoftKeyboard(NewStockReturnsChooseActivity.this);
