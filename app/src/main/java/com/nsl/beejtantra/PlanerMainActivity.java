    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (isAsync) {
                    Toast.makeText(getApplicationContext(), "Please wait some time..", Toast.LENGTH_LONG).show();
                    return;
                }
                finish();
            }
...
//get a list of installed apps.
    ...
    public void onBackPressed() {
        if (isAsync) {
            Toast.makeText(PlanerMainActivity.this, "Please wait some time..", Toast.LENGTH_LONG).show();
            return;
        }
        super.onBackPressed();
    ...
    private class Async_getallevm extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            ...
            try {
                ...
                String days = "60";
                String latestRecordDate = db.getLatestDateFromTable(TABLE_EMPLOYEE_VISIT_MANAGEMENT);
                if (latestRecordDate != null)
                    days = String.valueOf(Common.getNoOfdays(latestRecordDate));
                Common.Log.i("Planner URL " + Common.getCompleteURLEVM(Constants.NEW_OR_UPDATED_RECORDS_EMPLOYEE_VISIT_MANAGEMENT, userId, team) + "&days=" + days);
                Request request = new Request.Builder()
                        .url(Common.getCompleteURLEVM(Constants.NEW_OR_UPDATED_RECORDS_EMPLOYEE_VISIT_MANAGEMENT, userId, team) + "&days=" + days)
                        .get()
            ...
            if (jsonData != null) {
                try {
                    ...
                    JSONObject mainobj = jsonObject.getJSONObject("newrecord");
                    JSONArray companyarray = mainobj.getJSONArray("event_management");
                    ...
                    for (int n = 0; n < companyarray.length(); n++) {
                        ...
                        /*JSONArray adavancebooking = objinfo.getJSONArray("planner_approval");

                        for (int k = 0; k < adavancebooking.length(); k++) {
                            JSONObject planner_approval = adavancebooking.getJSONObject(k);
                        ...
                        if (cc.getCount() == 0) {
                            ...
                            db.addEVM(new Employe_visit_management_pojo(emp_visit_id, type, geo_tracking_id, user_id, customer_id,
                                    visit_plan_type, purpose_visit_titles, String.valueOf(purpose_visit), plan_date_time, concern_person_name, retailerId, farmerId, mobile, village, location_address, cropId, regionId, location_district, location_taluka, location_village, gstin, field_area, check_in_time,
