...
import com.nsl.beejtantra.SimpleImageActivity;
...
import static com.nsl.beejtantra.DatabaseHandler.KEY_TABLE_RETAILER_ID;
import static com.nsl.beejtantra.DatabaseHandler.KEY_TABLE_RETAILER_MASTER_ID;
import static com.nsl.beejtantra.DatabaseHandler.KEY_TABLE_RETAILER_MOBILE;
import static com.nsl.beejtantra.DatabaseHandler.KEY_TABLE_RETAILER_NAME;
import static com.nsl.beejtantra.DatabaseHandler.KEY_TABLE_RETAILER_PRIMARY_ID;
...
public class AllproductsActivity extends AppCompatActivity {
    ...
    TextView tvCompType, tvCompPer, tvCompRem, /*tvZone, tvDealer, tvRegion,*/
            tvPerLot, /*tvState,*/
            tvDateSowing, tvDateComplaint, tvInspectDate/*, tvDistrict*/;
...
//        statustext = (TextView) findViewById(R.id.statustext);
        ...
        tvCompType = findViewById(R.id.tv_comp_type);
        tvCompRem = findViewById(R.id.tv_comp_rem);
        tvCompPer = findViewById(R.id.tv_complaint_per);
       /* tvZone = findViewById(R.id.tv_zone);
        tvDealer = findViewById(R.id.tv_dealer);
        tvRegion = findViewById(R.id.tv_region);*/
        tvPerLot = findViewById(R.id.tv_performance_lot);
//        tvState = findViewById(R.id.tv_state);
        tvDateSowing = findViewById(R.id.tv_date_sowing);
        tvDateComplaint = findViewById(R.id.tv_date_complaint);
        tvInspectDate = findViewById(R.id.tv_inspect_date);
...
//        tv_village.setText(village);
        ...
        tvCompType.setText(getIntent().getExtras().getString("complaint_type", ""));
        tvCompPer.setText(getIntent().getExtras().getString("comp_per", ""));
        tvCompRem.setText(getIntent().getExtras().getString("comp_rem", ""));
//        tvZone.setText(getIntent().getExtras().getString("zone", ""));
//        tvDealer.setText(getIntent().getExtras().getString("dealer", ""));
//        tvRegion.setText(getIntent().getExtras().getString("region", ""));
        tvPerLot.setText(getIntent().getExtras().getString("per_lot", ""));
//        tvState.setText(getIntent().getExtras().getString("state", ""));
        tvInspectDate.setText(getIntent().getExtras().getString("insp_date", ""));
        tvDateSowing.setText(getIntent().getExtras().getString("date_of_sowing", ""));
        tvDateComplaint.setText(getIntent().getExtras().getString("date_of_comp", ""));
//        tvDistrict.setText(getIntent().getExtras().getString("district", ""));
...
//        }
    ...
    public class DisplayImages extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            imgfile = (getIntent().getExtras().getString("image", ""));
                    ...
                    public void run() {
                        ...
                        if (imgfile.length() > 0) {
                            ...
                            for (int i = 0; i < images.length; i++) {
                                frameViewsAll[i].setVisibility(View.VISIBLE);
                                Common.Log.i("Image " + i + " : " + Constants.IMAGE_URL + images[i]);
                                Glide.with(getApplicationContext())
                                        .load(Constants.IMAGE_URL + images[i])
                                        .asBitmap()
                                        .placeholder(R.mipmap.ic_launcher)
                                        .into(imageViewsAll[i]);

                                final int finalI = i;
                                imageViewsAll[i].setOnClickListener(new View.OnClickListener() {
                                    @Override
                                    public void onClick(View view) {
                                        Intent imageIntent = new Intent(AllproductsActivity.this, SimpleImageActivity.class);
                                        imageIntent.putExtra("index", finalI);
                                        imageIntent.putExtra("images", imgfile);
