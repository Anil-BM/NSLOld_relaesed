...
*/
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        boolean startedFromJob = intent.getBooleanExtra(JobScheduleService.STARTED_FROM,
                false);
...
*/
    ...
    private static void updateLocationInDB(Context context, String checkinlatlong) {
        ...
        String selectQueryss = "SELECT  " + KEY_TABLE_GEO_TRACKING_ID + "," + KEY_TABLE_GEO_TRACKING_ROUTE_PATH_LAT_LONG + "," + KEY_TABLE_GEO_TRACKING_FFMID + "," + PAUSE + "," + RESUME + " FROM " + TABLE_GEO_TRACKING + " where " + KEY_TABLE_GEO_TRACKING_USER_ID + " = " + Common.getUserIdFromSP(context) + " and  visit_date like '" + datefromcalander + "%'" + " ORDER BY " + KEY_TABLE_GEO_TRACKING_ID + " DESC LIMIT 1 ";
        try {
            ...
            if (ccc != null && ccc.moveToFirst()) {
                ...
                pauseP = ccc.getString(3);
                resumeP = ccc.getString(4);
                existedroutepath = String.valueOf(ccc.getString(1));
                if (existedroutepath == null || existedroutepath.equalsIgnoreCase("") || existedroutepath.equalsIgnoreCase("null") || existedroutepath.equalsIgnoreCase("0")) {
            ...
            String updatequerys = "UPDATE " + TABLE_GEO_TRACKING + " SET " + KEY_TABLE_GEO_TRACKING_ROUTE_PATH_LAT_LONG + " = '" + existedroutepath + "'," + SYNC_STATUS + " = 0" + " WHERE " + KEY_TABLE_GEO_TRACKING_ID + " = " + tid;
            if (existedroutepath != null && !existedroutepath.equalsIgnoreCase("") && existedroutepath.length() > 8)
            ...
            if (ccc != null)
                ccc.close();
            if (sdbw != null && sdbw.isOpen())
                sdbw.close();
        }

        if (Common.haveInternet(context)) {
            trackingId = sharedpreferences.getString("tracking_id", null);
            if (ffmID == null || ffmID == "0" || trackingId == null || existedroutepath == null || existedroutepath.length() < 8) {
                return;
            } else {
    ...
    public static boolean isCheckedIn(Context context) {
        ...
        try {
            ...
            sdbw1 = new DatabaseHandler(context).getWritableDatabase();

            ccc = sdbw1.rawQuery(selectQueryss, null);
            ...
            if (ccc != null && ccc.moveToFirst()) {
                ...
                if ((ccc.getString(3) == null || ccc.getString(3).equalsIgnoreCase("") || ccc.getString(3).equalsIgnoreCase("null")) && (ccc.getString(4) != null && ccc.getString(4).length() > 5)) {
                    ...
                    checkInTime = ccc.getString(4);
                    pauseFormDB = ccc.getString(5);
                    resumeFormDB = ccc.getString(6);
                    return true;
                }
    ...
    private void turnOnScreen() {
        ...
        if ((getSystemService(POWER_SERVICE)) != null) {
            ...
            screenLock.acquire(1 * 60 * 1000 /*10 minutes*/);
